/* $Id: xfonts.c,v 1.1 1999/08/27 16:49:59 dirk Exp $ */
...
/*
 * $Log: xfonts.c,v $
 * Revision 1.1  1999/08/27 16:49:59  dirk
 * Initial revision
 *
...
void
generate_font(FL_OBJECT *ob, fontdim_t *flist,
	        Font font, int first, int count, int listbase)
{
  ...
  if (!fs)
    {
      fprintf(stderr,"generate_font: Couldn't get font structure information\n");
      return;
    }
  ...
  for (i = 0; i < count; i++)
      ...
      if (debug_xfonts)
	{
	 ...
	 straight from the glXUseXFont(3) manpage.  */
      ...
      /* save width, height offset for later use (postscript rendering) */
      flist[c].width = width/2;
      flist[c].height = height/2;

      /* fprintf (stderr, isprint (c) ? "%c> " : "\\%03o> ", c);
	 fprintf(stderr, "width: %d height: %d\n", width, height); */

      x0 = - ch->lbearing  + width ? (width/2): 0;
      y0 = ch->descent - 1 + height ? (height/2): 0;
