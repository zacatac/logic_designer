...
parse_marc(int strucs) {
    ...
    fl_set_form_title(to_main.fd_main->main, "");
	...
	set_comment(to_main.comment, FL_RED, "Parse Error");
	free_struc_list();
	return 1;
    }
	...
	fl_set_form_title(to_main.fd_main->main, (&(the_title[1])));
	set_comment(to_main.comment, FL_BLUE, "Title: %s", (&(the_title[1])));
	...
	set_comment(to_main.comment, FL_RED, "Parse Error: no bases found in input file...");
	free_struc_list();
	return 1;
    }
	...
	dbDebug(DB_DEBUG, "the_brack: <%s>", the_brack);
        ...
        if(the_brack[0]=='#'){
	    set_comment(to_main.comment, FL_BLUE, "comment: %s", &the_brack[1]);
	    /* save for next round */
	...
	if(bracklen < old_bracklen) {
	    set_comment(to_main.comment, FL_BLUE, "Read incomplete structure file");
	    break;
	}
	...
	if ((s_i*100 / strucs) >= percent) {
	    set_comment(to_main.comment, FL_BLUE, "Read %d %% of structures...", percent);
	    percent += 10;
	}
    ...
    set_comment(to_main.comment, FL_BLUE, "Read 100 %% of structures...");
    dbDebug(DB_DEBUG, "<-parse_marc()");
...
parse_alain(int strucs) {
	...
	set_comment(to_main.comment, FL_BLUE, "Num de la structure lue : %d",s_i);
	...
	if(strchr(the_brack,'H') != 0){ /* complete structure ? */
		...
		set_comment(to_main.comment, FL_BLUE, "New synthesis");
	    ...
	    /* recuperation du commentaire */
            strncpy(alainstruc.info,strchr(the_bases,'|')+2,strpbrk(the_bases, "#")-strpbrk(the_bases, "|")-9);
...
parse_err(int s_i, int pos) {
  if(pos < 0) {
    set_comment(to_main.comment, FL_RED, "error in structure %i: too many opening braces", s_i);
  }
  else {
    set_comment(to_main.comment, FL_RED, "error in structure %i at position %i: closing brace", s_i, pos);
  }
    free_struc_list();
...
is_ct(FILE *fileptr) {
	...
	if(inp[0] == '<')
	    return 2; /* seems to be rnm DCSE format */
	else
